    ...
    for location_id in location_ids:
        try:
            ...
            for i in range(0, len(values) - window_size, step_size):
                ...
                features = values[i : i + window_size]
                # Get the corresponding timestamp for the last feature in the window
                feature_time = times[i + window_size]
                # Combine features, location_id, and timestamp
                row = np.append(features, [location_id, feature_time])
                rows.append(row)

            # Convert the list of rows into a DataFrame
            feature_columns = [
                f"{feature_col}_t-{window_size - i - 1}" for i in range(window_size)
            ]
            all_columns = feature_columns + ["location_id", "pickup_hour"]
            transformed_df = pd.DataFrame(rows, columns=all_columns)
